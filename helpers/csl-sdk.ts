...
export async function derivePrivateKeyAndAddressesFromMnemonic(
  ...
  try {
    ...
    const paymentCredential = CSL.Credential.from_keyhash(paymentPublicKey.hash());
    ...
    const baseAddress = CSL.BaseAddress.new(networkId, paymentCredential, stakeCredential);
    const paymentAddress = baseAddress.to_address().to_bech32();
    
    // Create stake address (stake credential only)
    const stakeAddress = CSL.RewardAddress.new(networkId, stakeCredential).to_address().to_bech32();
    
    console.log(`✅ Private key and addresses derived successfully for address index ${addressIndex}`);
...
export async function derivePrivateKeyFromMnemonic(
  ...
  const result = await derivePrivateKeyAndAddressesFromMnemonic(mnemonic, addressIndex);
  ...
  try {
    ...
    for (let i = 0; i < inputs.len(); i++) {
      ...
      deserialized.inputs.push({
        transactionId: Buffer.from(input.transaction_id().to_bytes()).toString('hex'),
        index: input.index(),
      });
    }
...
export async function signAndSubmitTransactionCSL(
  ...
  try {
    ...
    console.log(`✅ Transaction signed successfully. Signed CBOR length: ${signedTxHex.length}`);
    
    // deserialise the signed transaction and log for debugging purposes
    console.log(`🔍 DEBUG: Signed transaction: ${signedTxHex}`);
    deserializeTransactionCbor(signedTxHex);  

    // Submit the signed transaction using Blockfrost
    const { network } = await initializeCSL();
    const response = await fetch(`https://cardano-${network === "mainnet" ? "mainnet" : "preprod"}.blockfrost.io/api/v0/tx/submit`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Blockfrost submission failed: ${response.status} ${errorText}`);
    }
...
export async function signTransactionCSL(
  ...
  try {
    ...
    const privateKey = await derivePrivateKeyFromMnemonic(mnemonic, addressIndex);
